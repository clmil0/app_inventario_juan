...
✅ Lógica Contable Avanzada (Costo vs Venta): Implementada. Al realizar una venta, ahora se captura en la base de datos el `unit_cost` exacto de ese momento histórico, permitiendo calcular la ganancia real aunque el precio del producto cambie en el futuro.
✅ Revalorización de Inventario: Se creó la lógica para capitalización. Si se sube o baja el costo de un producto que ya tiene stock, el sistema calcula la diferencia y la registra automáticamente en una nueva tabla `inventory_revaluations`.
✅ Reporte PDF Tipo Estado de Cuenta: El cuadre contable exportable ahora es un desglose inteligente: si se filtra por "Hoy", detalla venta por venta; si es un periodo corto, agrupa el balance por día (Lunes, Martes); y si es mensual, agrupa por semanas. Se corrigió también el error del símbolo de moneda duplicado (`S/ S/`).
✅ Validación de Permisos SQL (RLS): Se detectó y solucionó un fallo silencioso al guardar precios. Se depuraron reglas RLS conflictivas y se aplicó una nueva política unificada, además del comando `GRANT ALL ON products TO authenticated;` para asegurar acceso de actualización al rol autenticado.
✅ Manejo de Errores en Supabase: Se agregaron interceptores de errores estrictos al bloque `update` de la tabla products (`confirmAddStock` en `admin.js`) para evitar que el sistema falle silenciosamente o registre eventos fantasma en el historial de auditoría.